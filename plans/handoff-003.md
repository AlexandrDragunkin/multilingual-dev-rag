...
#### 6c-4. Естественный killed>0 от переподключений VSCode (2026-07-21 10:54)

После запуска VSCode и открытия RAG-сессии (без синтетических инстансов)
`guard.log` зафиксировал **три естественных срабатывания за 90 секунд**
в живом VSCode-окружении. VSCode при подключении Claude-расширения
**переподключается несколько раз**, порождая новые MCP-инстансы, и
каждое новое подключение убирает прошлое:
...
Разбор:
- **10:53:24** — VSCode поднял первый MCP (50116). Других нет, killed=0.
- **10:54:13** — VSCode переподключился, новый real 22064 увидел прошлый
  real 19952 как дубль (общий предок Code.exe) и убил его. Заодно ушли
  transient bash-мусорные pids (13204, 17632 — артефакты моих прошлых
  команд в git-bash под ZCode, попавшие в snapshot в момент cleanup).
- **10:54:55** — третье переподключение, новый real 8512 убил прошлое
  22064.

**Это и есть целевой сценарий plan 003 в реальной работе** — защита от
накопления дублей от переподключений MCP-клиента. Подтверждает, что
проблема (накопление инстансов при переподключениях, handoff раздел 2)
существует не только как исторический замер 2026-07-20, но и
воспроизводится в реальном времени, и `process_guard` её корректно
решает. После стабилизации (последние 2 минуты наблюдения) держится
один инстанс 8512, без накопления.

**Наблюдение о VSCode-топологии:** в отличие от ZCode (где kill на Windows
не каскадируется и real остаётся висеть), VSCode при закрытии/смене
сессии сам каскадно гасит свою подсессию — сегодня при закрытии VSCode
все 3 его RAG-инстанса исчезли вместе с Code.exe. Но при **переподключении
внутри одной живой сессии** VSCode порождает новые MCP, не дожидаясь
естественной смерти старых — и здесь работает `process_guard`.
