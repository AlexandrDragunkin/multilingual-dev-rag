...
## 4. Что сделано (8 коммитов, 5 файлов + scripts)
...
Финальная ветка: 8 коммитов поверх `main` @ `b472078`.
...
`tests/test_process_guard.py` — **16 unit-тестов** (без `@pytest.mark.integration`,
...
**Вся сюита: `pytest tests/` → 86 passed** (было 70, +16). Прогон: `12.27s`.
...
- `pytest tests/test_process_guard.py -v` — 16/16 passed.
...
**Финальный dry-run** (select_victims от лица ZCode-инстанса):
- VSCode group (7580, 40556, 48504 — общий Code.exe) → схлопываются,
  выживает свежий, 2 старых — жертвы. Корректно.
- ZCode (34940) — отдельный клиент, никого не трогает в своей группе. Корректно.
