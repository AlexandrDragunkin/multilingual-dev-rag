...
> **АКТУАЛИЗАЦИЯ (2026-07-20).** PR #3 **уже слит в `main`** (merge `db19110`),
> ветка `fix/zvec-readonly` закрыта. Сверх того, что описано ниже в разделе 4,
> реализован и слит **«лучший» вариант дефекта 2** (диагностика в теле
> MCP-ответа, коммит `e49768e`) — см. раздел 7, пункт про дефект 2, он больше
> не «на будущее». Актуальная голова `main`: тесты **70 passed**. Разделы ниже
> сохранены как летопись состояния на момент открытия PR.
...
> **Дополнено после открытия PR (коммит `e49768e`, тоже в `main`).** Реализован
> «лучший» вариант дефекта 2: диагностический канал `_last_diagnostics` +
> `get_last_diagnostics()` в `zvec_searcher`, реэкспорт-заглушка для qdrant в
> `searcher.py`, вынос причины в тело MCP-ответа в `mcp_server.rag_search`.
> Юнит-тесты — `tests/test_diagnostics.py`. Сюита выросла до **70 passed**.
...
- ~~**Дефект 2 — «лучший» вариант из плана.**~~ **СДЕЛАНО** (коммит `e49768e`,
  слит в `main`). Выбран как раз «módul-level буфер последних ошибок»:
  `_last_diagnostics` в `zvec_searcher`, канал `get_last_diagnostics()`, вынос в
  тело MCP-ответа — контракт `search() -> list` не тронут. Осталось лишь
  **остаточное** наблюдение (вне scope 001): сбой не открытия, а `coll.query()`
  после успешного открытия по-прежнему молчит — см. «Итог» в
  `001-zvec-readonly-and-diagnostics.md`.
